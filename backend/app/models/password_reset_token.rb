...
# A single-use, time-limited password reset token. Only the SHA-256 digest is
# stored; the raw token is delivered solely in the emailed link. Valid means
# unconsumed and unexpired. Mirrors EmailVerificationToken.
class PasswordResetToken < ApplicationRecord
  ...
  def self.issue!(user)
    transaction do
      user.password_reset_tokens.unused.update_all(consumed_at: Time.current)
